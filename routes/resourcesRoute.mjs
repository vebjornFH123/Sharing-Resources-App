...
RESOURCE_API.post("/get", validateToken, async (req, res, next) => {
  try {
    ...
    res
      ...
      .send("Server cant get resources right now try again later")
      .end();
  }
});
...
RESOURCE_API.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        ...
        if (resource.id) {
          ...
          res
            ...
            .send(StatusCodes.resourceErrorResponse.failedToCreateResource)
            .end();
        }
      } else {
        res
      ...
      res
        .status(HTTPCodes.ServerSideErrorResponse.InternalServerError)
        .send("Server cant add resource right now try again later")
        .end();
    }
  }
);
    ...
    try {
      ...
      if (
        ...
        if (resource === 1) {
          ...
          res
            .status(HTTPCodes.SuccessfulResponse.Ok)
            .send(StatusCodes.resourceSuccessfulResponse.resourceUpdated)
            .end();
        } else {
          res
            .status(HTTPCodes.ClientSideErrorResponse.BadRequest)
            .send(StatusCodes.resourceErrorResponse.failedToUpdateResource)
            .end();
        }
      } else {
        res
      ...
      res
        .status(HTTPCodes.ServerSideErrorResponse.InternalServerError)
        .send("Server cant update resource right now try again later")
        .end();
    }
  }
);
...
RESOURCE_API.delete("/deleteResource", validateToken, async (req, res) => {
  try {
    ...
    if (deleteUser === 1) {
      res
        .status(HTTPCodes.SuccessfulResponse.Ok)
        .send(StatusCodes.resourceSuccessfulResponse.resourceDeleted)
        .end();
    } else {
      res
        .status(HTTPCodes.ClientSideErrorResponse.Conflict)
        .send(StatusCodes.resourceSuccessfulResponse.failedToDeleteResource)
        .end();
    }
  } catch (err) {
    ...
    res
      .status(HTTPCodes.ServerSideErrorResponse.InternalServerError)
      .send("Server cant delete resource right now try again later")
      .end();
  }
});
